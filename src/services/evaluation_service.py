...
from src.services.utils import get_object_or_404, safe_commit
...
from src.models.tasks import Task, TaskStatus, TaskExecutor
from src.models.users import UserRole, User
from src.schemas.evaluations import AvgEvaluationSchema
...
class EvaluationService:
    ...
    async def get_evaluation(session: AsyncSession, current_user: User, task_id: int, executor_id: int):
        ...
        if current_user.role not in (UserRole.admin, UserRole.manager) and task.creator_id != current_user.id:
    ...
    @staticmethod
    async def get_average(session: AsyncSession, current_user: User, user_id: int, start: str | None, end: str | None):
