...
import { getCreatedTasks } from "../../api/tasksApi.ts";
...
export default function CreatedTasksPage() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      try {
        ...
        alert("Ошибка загрузки созданных задач");
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  if (loading) return <div>Загрузка созданных задач...</div>;
    ...
    <Box sx={{ maxWidth: 700, mx: "auto", mt: 3 }}>
      ...
      {tasks.length === 0 ? (
        ...
        <Stack spacing={2}>
          {tasks.map((task) => {
            let statusText = "";
            let statusColor: "default" | "success" | "warning" | "info" | "error" = "default";
            let bgColor = "#f5f5f5";
